...
- [ ] **Step 2: 子集化 + 转 C（需 fonttools + lv_font_conv）**
```bash
...
- [ ] **Step 2: 实现 ui_home（4卡片+预览+时间轴+底栏）**
```c
